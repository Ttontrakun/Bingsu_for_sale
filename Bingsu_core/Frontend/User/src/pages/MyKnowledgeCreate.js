...
function MyKnowledgeCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('สร้าง Knowledge แล้ว', 'success');
      navigate(`/my-knowledge/${created.id}/add-data`, {
        state: { knowledgeName: created.displayName || knowledgeName.trim() },
      });
    } catch (err) {
      const msg = getErrorMessage(err);
      if (String(msg || '').includes('มีอยู่แล้ว') || err?.response?.status === 409) {
          ...
          <form noValidate onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-2.5 bg-yellow-400 hover:bg-yellow-500 text-gray-800 font-semibold rounded-lg disabled:opacity-50"
              >
                {loading ? 'กำลังสร้าง...' : 'สร้างและอัปโหลดเอกสาร'}
              </button>
            </div>
          </form>
        </div>
...
export default MyKnowledgeCreate;
